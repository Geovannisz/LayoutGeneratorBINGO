...
const DEFAULT_PARAMS = {
    ...
    hex_grid: { numRingsHex: 3, spacingFactorX: 2.0, spacingFactorY: 0.6667, centerExpScaleFactor: 1.0, addCenterTile: true, randomOffsetStddevM: 0.0, minSeparationFactor: 1.0 },
...
const PARAM_CONTROLS = {
    ...
    hex_grid: [ { id: 'numRingsHex', label: 'Nº Anéis Hex.', type: 'number', min: 0, max: 10, step: 1 }, { id: 'spacingFactorX', label: 'Fator Espaç. X', type: 'number', min: 0.1, max: 5, step: 0.1 }, { id: 'spacingFactorY', label: 'Fator Espaç. Y', type: 'number', min: 0.1, max: 5, step: 0.1 }, { id: 'centerExpScaleFactor', label: 'Fator Exp. Central', type: 'number', min: 0.5, max: 3, step: 0.05 }, { id: 'addCenterTile', label: 'Adicionar Tile Central', type: 'checkbox' }, { id: 'randomOffsetStddevM', label: 'Offset Aleatório (m)', type: 'number', min: 0, max: 1, step: 0.01 }, { id: 'minSeparationFactor', label: 'Fator Sep. Mín.', type: 'number', min: 0.5, max: 2, step: 0.05, condition: 'this.params.randomOffsetStddevM > 0' } ],
...
class AntennaLayoutGenerator {
    ...
    createTileLayout64Antennas(centerX, centerY) {
        ...
        // Adiciona o centro do tile (centerX, centerY) a cada uma das coordenadas base.
        return baseAntennaCoords.map(coord => [coord[0] + centerX, coord[1] + centerY]);
    }
    ...
    generateLayout() {
        ...
        try {
            ...
            switch (this.layoutType) {
                ...
                case 'hex_grid': this.currentLayout = window.BingoLayouts.createHexGridLayout(fullParams.numRingsHex, fullParams.tileWidthM, fullParams.tileHeightM, fullParams.spacingFactorX, fullParams.spacingFactorY, fullParams.centerExpScaleFactor, fullParams.addCenterTile, fullParams.randomOffsetStddevM, fullParams.minSeparationFactor, undefined, fullParams.centerLayout); break;
